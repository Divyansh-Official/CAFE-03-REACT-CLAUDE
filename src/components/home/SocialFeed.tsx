...
import { socialService, type InstagramPost } from '../../services/socialService';
...
export const SocialFeed: React.FC = () => {
  const { data: posts = [], isLoading } = useQuery({
    ...
    queryFn: socialService.getInstagramFeed,
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 mb-3">
            <img src={icons.instagram.url} alt="Instagram" className="w-7 h-7" />
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            {displayPosts.map((post: InstagramPost, i: number) => (
              <motion.a
                key={post.id}
                href={post.permalink}
                target="_blank"
              ...
              >
                ...
                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center gap-2">
                  {post.likes ? (
                    ...
                    <div className="text-white font-bold">View Post</div>
                  )}
                  <img src={icons.instagram.url} alt="Instagram" className="w-8 h-8 opacity-80" />
                </div>
              </motion.a>
            ))}
          </div>
        ) : (
          <div className="text-center py-10 rounded-2xl border border-white/10 bg-card/40">
            <p className="text-text-secondary">Instagram feed is unavailable right now.</p>
          </div>
        )}
        ...
        <div className="text-center mt-8">
          ...
          >
            <img src={icons.instagram.url} alt="Instagram" className="w-5 h-5" />
            Follow on Instagram
          </a>
        </div>
      </div>
